...
import Message from './qxt-vue/message/Message.vue'
import Dialog from './qxt-vue/dialog/Dialog.vue'
...
const message = ref([
  ...
  {
    type: 'notify',
    text: {type:'notify',text:'这是一个通知信息！'},
  },
  {
    type: 'notify',
    text: {type:'error',text:'这是一个错误信息！+'},
  },
  {
    type: 'image',
...
<template>
  <div class="flex flex-col gap-4 p-5 max-w-md m-auto bg-gray-200">
    <Message v-for="item in message" v-bind="item" avatar-class="size-10 rounded-lg"></Message>
  </div>
  <Dialog title="对话框标题" content="这是一个" :actions="[{text:'确定',class:'bg-blue-500 text-white'},{text:'取消',class:'bg-gray-500 text-white'}]">
    <template #trigger>
      <Button>打开对话框</Button>
    </template>
  </Dialog>
