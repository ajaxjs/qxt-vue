...
import type { PickerItem } from './type.ts';
...
//const { startYear = 1900, endYear = 2100, format = 'lifa YYYY-MM-DD HH:mm leap', type = 'datetime' } = defineProps<IDateViewProps>();
...
const format = computed(() => {
    ...
    return type == 'date' ? 'lifa YYYY-MM-DD leap' : 'lifa YYYY-MM-DD HH:mm leap';
});

console.log('type',props.type,format.value);


const emit = defineEmits(['update:modelValue', 'change']);
const columns = ref()
const dateValue = defineModel<string>();
const date = reactive(new TDate(dateValue.value, format.value));
...
function buildDateColumn() {
    ...
    return fkey!.map(key => {
        if (['YY', 'YYYY'].includes(key)) {
            return buildYears(props.startYear, props.endYear);
